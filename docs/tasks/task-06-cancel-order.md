...
- [x] Integration test: two concurrent cancels of the same order → both return 200, status
...
- [x] Cancel is covered by a test that asserts the `UPDATE ... WHERE status='Confirmed'` guard
